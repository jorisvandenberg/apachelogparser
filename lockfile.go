...
func lockUnlock(lock bool, args Args) {
    ...
    if lock {
        ...
        // Register a deferred function to release the lockfile when the program exits.
        defer func() {
            // Close the lockfile.
    ...
    // Register a signal handler to release the lockfile when the program receives a termination signal.
