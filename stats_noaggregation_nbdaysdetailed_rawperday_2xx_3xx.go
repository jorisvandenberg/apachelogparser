...
func noaggregation_nbdaysdetailed_rawperday_2xx_3xx(args args) {
	logger("i'm going to generate a table and a linechart with the dayly raw hits")
	...
	myTable := Table{
		Pagetitle:       "Number of raw 2xx and 3xx hits per day over th last " + strconv.Itoa(args.outputs.number_of_days_detailed) + " days",
	...
	createtable(args, "noaggregation_nbdaysdetailed_rawperday_2xx_3xx_table.html", "table of the raw 2xx and 3xx per day over the last "+strconv.Itoa(args.outputs.number_of_days_detailed)+" days", myTable, "hits", 3)
	PreChartText = ""
	PostChartText = ""
	createlinegraph(XValues_linegraph, YValues_linegraph, "line graph of the raw hits with status 2xx and 3xx", "Count of all raw succesfull hits (filtering out all 4xx and 5xx return codes).", args, "noaggregation_nbdaysdetailed_rawperday_2xx_3xx_linegraph.html", "hits", 3)
	logger("finished generating a table and a linechart with the dayly raw hits")
}
