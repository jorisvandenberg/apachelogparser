...
func noaggregation_nbdaysdetailed_uniqueperday_2xx_3xx(args args) {
	logger("i'm going to generate a table and a linegraph containing a dayly grouping of unique hits")
	...
	myTable := Table{
		Pagetitle:       "Number of unique 2xx and 3xx hits per day over th last " + strconv.Itoa(args.outputs.Number_of_days_detailed) + " days",
	...
	createtable(args, "noaggregation_nbdaysdetailed_uniqueperday_2xx_3xx_table.html", "table of the unique 2xx and 3xx per day over the last "+strconv.Itoa(args.outputs.Number_of_days_detailed)+" days", myTable, "hits", 3)
	PreChartText = ""
	PostChartText = ""
	...
	logger("finished generating a table and a linegraph containing a dayly grouping of unique hits")
}
