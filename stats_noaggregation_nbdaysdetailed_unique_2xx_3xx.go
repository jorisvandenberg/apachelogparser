...
func noaggregation_nbdaysdetailed_unique_2xx_3xx(args Args) {
	...
	if check_if_stats_is_slice.Kind() == reflect.Ptr && check_if_stats_is_slice.IsNil() {
		logger("i wanted to verify if i had to create stats with the hourly unique hits, but it seems like all Stats are disabled in the config")

	} else if check_if_stats_is_slice.Kind() == reflect.Slice {
		for _, curstat := range args.Stats {
			if curstat.Statname == "stat_perhour_hits_unique_2xx_3xx" {
				foundcurstat = true
				mycurstat = curstat
			}
	...
	if foundcurstat {
		...
		logger("i'm going to generate a table and/or a linechart (depending on the config) with the hourly unique hits")
		...
		logger("finished generating a table and/or a linechart with the hourly unique hits")
	} else {
		logger("i could not find this stat stat_perhour_hits_unique_2xx_3xx in the config. Is it disabled?")
	}
}
