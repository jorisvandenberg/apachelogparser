...
func genstats(args Args, string_for_log string, statname_from_conf string, querydb_key string, parameters []interface{}, tableheaders map[string]string, xaxisfields []int, valuefield int, legende string, first_3_records_year_month_day_for_limiting bool) bool {
	...
	for _, current_x_days_weeks_months := range compare_x_days_weeks_months_config_struct_slice {
		...
		var XValues_linegraph_4weekcomp []string
		XValues_linegraph_4weekcomp = append(XValues_linegraph_4weekcomp, "now")
		myCounter := 3
		for myCounter <= current_x_days_weeks_months.Number_of_days_weeks_months_compare_nbitems_inloop {

			XValues_linegraph_4weekcomp = append(XValues_linegraph_4weekcomp, current_x_days_weeks_months.What_hours_days_weeks_months+"-"+strconv.Itoa(myCounter-2))
			myCounter += 1
		}
